...
def _enrich_posts(
    ...
    resolver: Resolver,
    team_by_post: Optional[dict[str, str]] = None,
) -> list[dict]:
...
def pinned(state, channel, limit):
    ...
    if not state.human:
        click.echo(json.dumps(_enrich_posts(posts, authors, resolver), indent=2))
    else:
...
def members(state, channel):
    ...
    # Sort: online first, then away, then offline/dnd
